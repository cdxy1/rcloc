...
fn resolve(
    ...
    Ok(match language {
        ...
        "C#/Smalltalk" => {
            let resolved = csharp_or_smalltalk(&lines)?;
            // A tool-generated C# file announces itself in the first two
            // lines, and gets its own language so it can be told apart --
            // or dropped outright under --no-autogen.
            if resolved == "C#" && is_autogenerated_csharp(&lines)? {
                if opts.no_autogen {
                    return Ok(ignored(
                        "--no-autogen ignores this auto-generated C# file".to_string(),
                    ));
                }
                lang("C# Generated".to_string())
            } else {
                lang(resolved)
            }
        }
...
/// `// <auto-generated>` within the first two lines, which is the marker
/// Visual Studio and friends emit.
fn is_autogenerated_csharp(lines: &[String]) -> Result<bool> {
    let head: String = lines.iter().take(2).cloned().collect();
    Ok(re(r"//\s+<auto-generated>")?.is_match(&head)?)
}
