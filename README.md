...
- Comment scanners do not understand string literals, so `"/* x */"` inside a
  string is stripped.
- Perl's `$1` survives a *failed* match but is cleared by a *successful* one
  with no capture groups. The `remove_between_*` family reads `$1` across
  lines and depends on this.
